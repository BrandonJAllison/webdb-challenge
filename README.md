...
1. Explain the difference between `RDBMS` and `SQL`.
  ...
  RDBMS is the relational model of data where SQL is the query language use to access that data from the database.

1. Why do tables need a `primary key`?

    It is needed to uniquely identify all table records in the relational database model system.

1. What is the name given to a table column that references the primary key on another table.

    A foreign key is the name given to a column that references the primary key on another table.

1. What do we need in order to have a _many to many_ relationship between two tables.

    A bridge table is needed for many to many relationships between tables.  The bridge table will contain the foreign key from the related tables.
...
complete: Use `knex` to add _data seeding_ scripts for projects and actions.
